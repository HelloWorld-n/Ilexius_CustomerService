		...
		<table class="tg">
			...
			<tbody>
				<tr>
					...
					<td class="tg-td0d subject">{HtmlTableUtil.html_escape(ticket["subject"])}</td>
					<td class="tg-td0d name">{HtmlTableUtil.html_escape(ticket["name"])}</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td class="tg-td0d">
						<form id="alter{formCounter}" method="post" action="viewSingleTicket.py">
...
<body>
	...
	<h1><a href="index.py">Main</a> / View tickets</h1>
	<h4>Sort By</h4>
	<div>
		<form method="post" action="viewTickets.py">
...
if True:
		...
		try:
			...
			query = "SELECT * FROM tickets WHERE hidden=False "
